...
**Domains:**
- [ramilconnect.ai](https://ramilconnect.ai) – App
- [ramilconnectadmin.ai](https://ramilconnectadmin.ai) – Admin
...
```
*Note: In non‑production mode, starting the server automatically creates database tables and seeds the initial admin account (`admin@ramilconnect.ai` / `admin123`).*

### Running Tests
We use `pytest` for integration testing. Since the app relies on `pgvector` for chat embeddings, a running PostgreSQL database is required for the full test suite.
```bash
